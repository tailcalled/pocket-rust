...
- `while let PAT = SCRUT { BODY }` (and the labeled form) — desugared at parse time in `parse_while_expr` to `while true { if let PAT = SCRUT { BODY } else { break; } }`. SCRUT is re-evaluated each iteration; loop exits on the first iteration where PAT doesn't match. Typeck and codegen see only the synthesized while + if-let, so all the existing pattern/break/continue/label machinery just works. The label, if any, propagates onto the synthesized WhileExpr.
