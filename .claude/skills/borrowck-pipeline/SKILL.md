...
description: Use when modifying or debugging pocket-rust's borrow checker (`src/borrowck/`). Covers the CFG submodule layout, the four-stage dataflow (build → moves → liveness → borrows), NLL semantics, move/copy classification, raw-pointer deref behavior, and reborrow patterns the borrow checker recognizes.
---

# borrowck pipeline

Entry: `pub fn check(&Module, &StructTable, &EnumTable, &TraitTable, &mut FuncTable) -> Result<(), Error>` in `src/borrowck/mod.rs`. A thin per-function driver that walks every function body, hands it to the CFG pipeline, and writes the resulting `moved_places`/`move_sites` back onto each `FnSymbol`/`GenericTemplate` for codegen's drop-flag synthesis.
