...
**APIT (argument-position `impl Trait`):** `fn apply(f: impl Fn(u32) -> u32) -> u32` parses to `TypeKind::ImplTrait(bounds)` and the parser's post-params `desugar_apit` step rewrites it into a fresh anonymous type-param `__impl_<n>` with the recorded bounds, appended to `Function.type_params`. Calls dispatch the same way as an explicit `<F: Fn(...)>` declaration. Bare-call `f(args)` against an APIT-typed local routes through `check_bare_typeparam_fn_call` (sibling of `check_bare_closure_call`): looks up the Fn signature on the type-param's bound via `typeparam_fn_signature` (reads `ctx.type_param_bounds` / `type_param_bound_args` / `type_param_bound_assoc`), unifies the arg tuple, and records a `Fn::call` trait dispatch with `recv_type = Param(name)`.

**RPIT (return-position `impl Trait`):** existential, NOT a type parameter — the function (not the caller) chooses the concrete type. Implementation:
- `RType::Opaque { fn_path, slot }` and `InferType::Opaque { fn_path, slot }` represent the existential. `slot` disambiguates multiple `impl Trait` occurrences in the same return signature (e.g. `fn f() -> (impl A, impl B)` produces two distinct slots).
- Setup's `rewrite_rpit_in_type` walks the declared return type, replacing each `TypeKind::ImplTrait(bounds)` with a synth `Path("__rpit_slot_N")` and collecting the bounds. `resolve_type` runs over the rewritten type with the synth names added to `type_params` (so they resolve to `Param("__rpit_slot_N")`). A post-pass `substitute_rpit_synths_to_opaque` swaps each synth `Param` for `Opaque{fn_path, slot}`. The resolved bounds land on `FnSymbol/GenericTemplate.rpit_slots: Vec<RpitSlot { bounds, pin: Option<RType> }>`.
- Body check (`check_block`) uses `rtype_to_infer_with_opaque_vars` to convert the expected return RType to InferType, allocating a fresh inference Var per `Opaque{slot}`. Standard unification then pins each Var from the body's actual return shape. After unify, each Var resolves to a concrete sub-type — that's the slot's pin. The pin is validated against the slot's bounds via `solve_impl_in_ctx_with_args` and recorded on the FnSymbol/Template.
- Per-fn finalize (`finalize_rpit_for_one_function`) runs at the end of `check_function`: substitutes `Opaque{this_fn, slot} → pin` in this fn's own return_type. So callers checked later in the crate see the concrete type at call sites. Crate-level `finalize_rpit_substitutions` after `check_module` finishes the same substitution across all functions for any cross-fn opaques.
- Lifetime bounds (`impl Trait + 'a`) parse via `parse_type`'s impl branch (lifetime tokens after `+` are consumed and discarded — Phase B structural carry only).
- Non-RPIT `impl Trait` positions (struct fields, type aliases, raw return type without going through the rewrite) are rejected at `resolve_type` with "`impl Trait` is only allowed in argument position".
...
- **Return-position `impl Trait`** (RPIT): existential opaque type via `RType::Opaque{fn_path, slot}` — see APIT/RPIT section above.
