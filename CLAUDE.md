...
- Function body: a block — a sequence of `let` statements followed by an optional tail expression. (No expression statements yet, no nested blocks.)
- Statements: `let NAME = EXPR;` and `let NAME: TYPE = EXPR;`. The optional type annotation is checked against the value's inferred type. The bound name is in scope for subsequent statements and the tail expression. No `mut`, no patterns, no shadowing checks.
...
Local types from `let` statements are computed during typeck (in declaration order) and stored on `FnSymbol.let_types`. Borrowck reads them so it knows whether each binding is `Ref`-typed (in which case reads of that binding don't record moves). Codegen recomputes the types as it walks (since `codegen_expr` already returns the type of each expression) and uses them to size the WASM locals it allocates for each binding.
