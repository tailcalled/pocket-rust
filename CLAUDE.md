...
The remaining looseness vs Rust: reads of `Copy`-typed owned locals (e.g. `usize`) are still tracked as moves. So `f(a, a)` where `a: usize` rejects in pocket-rust though Rust would copy. Fixing this is a separate pass — extend the "ref locals don't move on read" rule in `walk_var` / `walk_field_access` to `is_copy(rtype)`. Several existing error tests rely on the strict behavior and would need updating.
