...
- Types: integers (u8, i8, u16, i16, u32, i32, u64, i64, u128, i128, usize, isize), structs, `&T`, and `&mut T`. WASM layout: ≤32-bit integers (and usize/isize, since we target wasm32) flatten to 1 `i32`; 64-bit integers flatten to 1 `i64`; 128-bit integers flatten to two `i64`s (low half then high half). Structs flatten in declaration order, recursively — a `Point { x: u32, y: u64 }` is `[i32, i64]`. `&T` and `&mut T` have the same layout as `T` (refs are a pure compile-time concept; we have no linear memory). All function params and return values that flatten to more than one WASM scalar become multi-value signatures.
...
- References: shared `&T` and unique `&mut T` references are allowed in parameter types and as `&expr` / `&mut expr` expressions. Both kinds are forbidden in struct fields and return types (sidesteps lifetime annotations). Field access through a reference is allowed only for `Copy` fields (`usize`, `&U`, `&mut U`); accessing a struct field through a reference is otherwise rejected as "cannot move out of borrow". There is no explicit dereference operator (`*r`); auto-deref handles `r.field` and `r.field = …` (the latter only through `&mut`). Borrow conflicts (two `&mut`, or `&mut` + `&` on overlapping places) are rejected at borrowck.

`&mut` codegen: refs have no runtime representation, so we can't pass a "pointer". Instead the compiler does two cooperating tricks. (1) **Same-function alias.** `let r = &mut p;` does not allocate new locals — `r`'s WASM-local range is set equal to the source place's range, so writes through `r` go directly into the source's locals. The value expression is still walked (so nested `let_idx`/`lit_idx` counters stay in sync) but its pushed values are dropped. (2) **Out-parameter rewriting at call sites.** A function with `&mut T` parameters is compiled to return its `&mut T` parameters' final values alongside its normal return: signature `(R_flat, T1_flat, T2_flat, …)`. The function emits `LocalGet` of each `&mut` param's flat locals after the body. The caller, before invoking, computes each `&mut` arg's source range; after the call, it pops the modified values back into those source locals (in reverse order, since multi-return is stack-shaped). To make `let r = &mut p; f(r);` work without re-borrowing, borrowck treats reads of a `&mut`-typed `Var` as **moves**: the binding's borrow holds are transferred to the consumer (the call slot or the new binding) and the binding itself is marked moved. Codegen meanwhile sees that `r`'s alias range *is* `p`'s range, so the writeback after the call lands on `p` directly.
